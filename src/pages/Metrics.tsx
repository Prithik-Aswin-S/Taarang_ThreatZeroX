...
import { BarChart3, Grid3x3, Target, TrendingUp, Activity, Gauge, RefreshCw, Download } from "lucide-react";
    ...
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Precision, recall, FPR, F1, and confusion matrix across all detection challenges</p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={exportCsv} disabled={results.length === 0} className="rounded-lg px-4 py-2 text-sm flex items-center gap-2 border border-slate-600 text-slate-300 hover:bg-slate-700/30">
      ...
      {error && (
        <div className="glass rounded-lg p-3 border-red-500/40 text-sm text-red-300">
          {error}
        </div>
      )}

      {loading ? (
        <div className="glass rounded-xl p-8 text-center text-slate-400">Running all reference rules against datasets...</div>
      ) : (
        <>
          {/* Overall metrics */}
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <MetricCard label="Precision" value={`${(overallPrecision * 100).toFixed(1)}%`} tone={overallPrecision >= 0.8 ? "success" : "warning"} icon={<Target className="h-4 w-4" />} />
            <MetricCard label="Recall" value={`${(overallRecall * 100).toFixed(1)}%`} tone={overallRecall >= 0.8 ? "success" : "warning"} icon={<TrendingUp className="h-4 w-4" />} />
            <MetricCard label="False Positive Rate" value={`${(overallFPR * 100).toFixed(1)}%`} tone={overallFPR < 0.1 ? "success" : "threat"} icon={<Activity className="h-4 w-4" />} />
            <MetricCard label="F1 Score" value={`${(overallF1 * 100).toFixed(1)}%`} tone="cyber" icon={<Gauge className="h-4 w-4" />} />
          ...
          <Panel title="Per-Challenge Detection Performance" icon={<BarChart3 className="h-4 w-4 text-cyan-400" />}>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-[11px] uppercase text-slate-500 border-b border-slate-700/40">
                    <th className="text-left py-2 px-2">Challenge</th>
                    <th className="text-left py-2 px-2">MITRE</th>
                    <th className="text-center py-2 px-2">TP</th>
                    <th className="text-center py-2 px-2">FP</th>
                    <th className="text-center py-2 px-2">TN</th>
                    <th className="text-center py-2 px-2">FN</th>
                    <th className="text-center py-2 px-2">Precision</th>
                    <th className="text-center py-2 px-2">Recall</th>
                    <th className="text-center py-2 px-2">FPR</th>
                ...
                <tbody>
                  {results.map((r) => (
                    <tr key={r.challengeId} className="border-b border-slate-700/20 hover:bg-slate-700/10">
                      <td className="py-2.5 px-2 text-slate-200">{r.title}</td>
                      <td className="py-2.5 px-2"><Badge tone="cyber">{r.mitre}</Badge></td>
                      <td className="py-2.5 px-2 text-center text-emerald-300">{r.result.confusion_matrix.true_positives}</td>
                      <td className="py-2.5 px-2 text-center text-amber-300">{r.result.confusion_matrix.false_positives}</td>
                      <td className="py-2.5 px-2 text-center text-cyan-300">{r.result.confusion_matrix.true_negatives}</td>
                      <td className="py-2.5 px-2 text-center text-red-300">{r.result.confusion_matrix.false_negatives}</td>
                      <td className={`py-2.5 px-2 text-center ${r.result.precision >= 0.8 ? "text-emerald-400" : "text-amber-400"}`}>{(r.result.precision * 100).toFixed(0)}%</td>
                      <td className={`py-2.5 px-2 text-center ${r.result.recall >= 0.8 ? "text-emerald-400" : "text-amber-400"}`}>{(r.result.recall * 100).toFixed(0)}%</td>
                      <td className={`py-2.5 px-2 text-center ${r.result.false_positive_rate < 0.1 ? "text-emerald-400" : "text-red-400"}`}>{(r.result.false_positive_rate * 100).toFixed(0)}%</td>
                      <td className="py-2.5 px-2 text-center text-slate-300">{(r.result.f1_score * 100).toFixed(0)}%</td>
                      <td className="py-2.5 px-2 text-center text-slate-500">{r.result.execution_time_ms}ms</td>
                    </tr>
                  ))}
                </tbody>
              </table>
